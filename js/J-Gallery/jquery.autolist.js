	...
	return this.each(function() {
		...
		function move(){
			...
			if(options.link !== false)
				$(sel).find(".link").attr("href", $(sel).find(".img_box > div:eq("+trigger+") img").attr("link")).attr('target', $(sel).find(".img_box > div:eq("+trigger+") img").attr("target"));
		};
		...
		function play(){
			timeSet = setTimeout(go, options.timeout);
		}
