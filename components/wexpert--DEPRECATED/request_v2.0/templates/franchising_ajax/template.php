...
<div class="ajax_form_wrapper">
	...
	<? else:?>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td>
					<label for="surename"><?=GetMessage('RT_surname')?> <span class="req">*</span></label>
					<br />
					<input type="text" maxlength="450"
						class="req_inpt"
						name="<?=$arParams['POST_NAME']?>[surname]"
						value="<?=($arParams['_POST'][ $arParams['POST_NAME'] ]['surname']) ? $arParams['_POST'][$arParams['POST_NAME']]['surname'] : '';?>"
					/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td>
					<label for="city"><?=GetMessage('RT_franch')?> <span class="req">*</span></label>
					<br />
					<select name="<?=$arParams['POST_NAME']?>[franch]" class="req_inpt">
						<option value=""></option>
						<?foreach ($arResult['CUSTOM_SELECTS']['FRANCH'] as $franch) {?>
							<option <?if ($arParams['_POST'][ $arParams['POST_NAME'] ]['franch'] == $franch['ID']) {?> selected="selected" <?}?>
								value="<?=$franch['ID']?>"><?=htmlspecialcharsEx($franch['NAME'])?></option>
						<?}?>
					</select>
				</td>
			</tr>
			<tr>
				<td>
					<label for="city"><?=GetMessage('RT_volume')?> <span class="req">*</span></label>
					<br />
					<input type="text" maxlength="450"
						class="req_inpt"
						name="<?=$arParams['POST_NAME']?>[volume]"
						value="<?=($arParams['_POST'][ $arParams['POST_NAME'] ]['volume']) ? $arParams['_POST'][$arParams['POST_NAME']]['volume'] : '';?>"
					/>
				</td>
			</tr>
			<tr>
				<td>
					<label for="city"><?=GetMessage('RT_phone')?> <span class="req">*</span></label>
					<br />
					<input type="text" maxlength="450"
			...
			<tr>
				<td>
					<label for="msg"><?=GetMessage('RT_msg')?></label><br>
					<textarea name="<?=$arParams['POST_NAME']?>[msg]"><?=($arParams['_POST'][ $arParams['POST_NAME'] ]['msg']) ? $arParams['_POST'][$arParams['POST_NAME']]['msg'] : '';?></textarea>
				</td>
			</tr>
