...
if (! function_exists('CustomRequestMailVars_franchising')) {
	/**
	 * Дополнительный обработчик для формы $arParams["POST_NAME"] == franchising
	 * @param array $_post массив после успешного выполнения $arParams[_POST]
	 * @param array $arResult массив с данными, у него есть ключ $arResult['CUSTOM_SELECTS']
	 * 	из обработчиков произвольных выборок ;)
	 * @param array &$mailVars массив $mailVars, можно модифицировать
	 */
	function CustomRequestMailVars_franchising($_post, $arResult, &$mailVars)
	{
		$customSelect = $arResult['CUSTOM_SELECTS'];
		$franchIndex = $customSelect['FRANCH_INDEX'][ $mailVars['franch'] ];
		$mailVars['franch_name'] = $customSelect['FRANCH'][$franchIndex]['NAME'];
	}
}
?>