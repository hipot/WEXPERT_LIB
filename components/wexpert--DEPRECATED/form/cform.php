...
class CFormBuilder {
	 ...
	 * @param bool $scripts Выводить ли сразу срипты или вернуть return
	 */
	public function End($scripts=true){
	...
	/**
	 * Возвращает массив значений всех полей 'код_поля' => 'значение'
	 * @return mixed
	 */
	 ...
	 */
	public function ShowErrors($ret=false){
		$errors = array();
		foreach($this->cfg['f'] as  $f){
	...
	protected function putHeadScripts(){
		...
		if(!self::$jsHeadShowed){
			$file = str_replace($_SERVER['DOCUMENT_ROOT'],'',dirname(__FILE__)).'/validators.js';
			if($APPLICATION && method_exists($APPLICATION, 'AddHeadScript')){
				$APPLICATION->AddHeadScript($file, true);
			} else{
	...
	protected function putScripts(){
		...
		foreach($this->cfg['f'] as &$ar){
			...
			if(is_array($ar['validate'])){
				foreach($ar['validate'] as $k => $v){
					$ob=$this->getOb($ar, $cl, $mt, $args, $v, $k);
					$vout='false';
					if(is_array($v)){
						$vout ='{';
						$tm=array();
					...
					$fn=$mt.'_error';
					$erm=$ob->$fn();
					$fuln=str_replace('[','', str_replace(']','', $ar["full_name"]));
					$id = "$cl$mt$fuln";
					$out .= "\tif(window['$cl']['$mt'] !== undefined)\n";
					$out .= "\t\twindow['$cl']['$mt'].call($(\"[name='$ar[full_name]']\"), '$erm', '$id', $vout);\n";
				}
			} else{
				$ob=$this->getOb($ar, $cl, $mt, $args);
				$fn=$mt.'_error';
				$erm=$ob->$fn();
				$fuln=str_replace('[','', str_replace(']','', $ar["full_name"]));
				$id = "$cl$mt$fuln";
				$out .= "\tif(window['$cl']['$mt'] !== undefined)\n";
				$out .= "\t\twindow['$cl']['$mt'].call($(\"[name='$ar[full_name]']\"), '$erm', '$id');\n";
			}
		}
		$out .= "});\n</script>";
		return $out;
...
class CFValidators{
	...
	/**
	 * Валидатор stringsize(3) <br>
	 * Максимальное количество символов <br>
	 * CFValidators::stringsize => array(3)
	 */
	const stringsize = 'CFValidators::stringsize';
	...
	/**
	 * Валидатор stringsize
	 * @param $ar Массив параметров
	 */
	public function stringsize($ar) {
		if(strlen($this->cfg['value'])>$ar[0]){
			$this->set_error(false,false,'',$ar[0]);
		}
	}

	/**
	 * Ошибка валидатора stringsize
	 * @return string
	 */
	public function stringsize_error() {
		$prms = func_get_args();
		$ss = ($prms[0])?': '.$prms[0]:'';
		return CFTypes::varsReplacer($GLOBALS['MESS']['CFB_fld_string_max_sz'], array('label' => $this->cfg['label'], 'size'=> $ss));
	}
	 ...
	 */
	protected function set_error($str=false, $id=false, $sfx='', $prm=false){
		if(!$str){
			$bgt = debug_backtrace();
...
$MESS['CFB_fld_mail'] = 'Поле "#label#" не заполнено либо заполнено неправильно';
...
$MESS['CFB_fld_string_max_sz'] = 'Строка "#label#" превышает максимально допустимый размер#size#';
?>
