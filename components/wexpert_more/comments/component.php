...
if(!$arParams['ELEMENT_ID']){
	ShowError('Необходимо указать идентификатор комментируемого элемента!');
	return false;
}
...
$arParams['MAX_RATE_FROM_IP'] = $arParams['MAX_RATE_FROM_IP']?$arParams['MAX_RATE_FROM_IP']:1;
$arResult['ALL_COMMENTS_IS_HIDDEN'] = true;

$CACHE_ID = 'we_comments_page' . md5(serialize($arParams).$USER->GetID());
$CACHE_DIR = 'php/we_comments/';
...
if($_GET['install_me_please']=='Y'){
	if($tbls->AffectedRowsCount()<1){
		...
		ShowError($arrErrors);
		$rsT = CEventType::GetList(array('TYPE_ID'=>'WE_COMMENTS_ADDED'));
...
// конец установки ==============================================================
	...
	if(check_bitrix_sessid() && !empty($_POST['arCommentFields']) && $_POST['arCommentFields']['DEL']!='Y'){
		...
		if(check_bitrix_sessid() && $arParams['PRE_MODER']=='Y' && ($_POST['arCommentFields']['STATUS']=='' || !isset($_POST['arCommentFields']['STATUS']))){
	...
	if(!empty($arResult['errors']) || !empty($_POST['arCommentFields'])){
		$obMenuCache->CleanDir($CACHE_DIR); // сбрасываем кеш
	}
	...
	if ($obMenuCache->StartDataCache($arParams["CACHE_TIME"], $CACHE_ID, $CACHE_DIR)) {
		...
		while ($arItm = $res->Fetch()) {
			if($arParams['USE_RATE'] == 'Y'){
				$arItm['RATE'] = WeCommentsRate::GetByComment($arItm['ID']);
			}
			if($arItm['STATUS']=='')
				$arItm['STATUS'] = 'P';
			if($arItm['STATUS'] == 'P'){
				$arResult['ALL_COMMENTS_IS_HIDDEN'] = false;
			}
		...
		$arResult = array_merge($arResult, $arVars["arResult"]); // в удобную переменную
	}

	foreach($arResult['COMMENTS'] as $c){
		if($c['STATUS'] == 'P'){
			$arResult['ALL_COMMENTS_IS_HIDDEN'] = false;
		}
	}
