 ...
 *
 * Необходимо:
 * php 5.4, Fileinfo Extension, GD (лучше Imagick)
 *
 * @see http://image.intervention.io/
 * @see http://hipot.wexpert.ru/Codex/cimg-constantly-integrable-modifier-of-graphics/
 *
...
class weImg
{
	...
	protected function load($img)
	{
		if (is_numeric($img)) { 												// если входит БитриксID картинки
			...
			if (! is_file($img)) {
				throw new \weImgException('wrong_input_img_type');
			}
			$this->path_type		= 'abs';
			$this->src				= $img;
	...
	{
		if ($method === false) {
			throw new \weImgException('no_resize_method_set');
		}
	...
	public static function ResizeOverlay($f, $to, $pos = 'center', $w, $h, $m, $retAr = false)
	{
		...
		if (! is_file($to)) {
			throw new \weImgException('wrong_image_wm ' . $to);
			return false;
		}
...
class weImgException extends \Exception
{
} // end class
