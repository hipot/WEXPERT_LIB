...
class PhpCacher
{
	 ...
	 */
	static function clearDirByTag($tagName)
	{
	 ...
	 *
	 * @param string $tagName
	 * @param bool $relRootPath - false - относительно '/bitrix/cache/',
	 ...
	 */
	static function returnCacheDataAndSave($tagName, $cacheTime, $callbackFunction, $params = array())
	{
		self::$LAST_ERROR = '';
		...
		if (($cacheTime = intval($cacheTime)) < 0) {
		...
		if ($obCache->StartDataCache($cacheTime, $CACHE_ID, $CACHE_DIR)) {
			...
			self::$params = $params;
			// TODO если передавать через array($params) получаем сразу захват всех параметров
