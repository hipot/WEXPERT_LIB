...
/**
 * Класс для работы с заказами/корзинам
 *
 * Просьба любые изменения согласовывать с автором - crus (wexpert)
 *
 * @author wexpert, 2014
 * @version 1.2
 */
class SaleUtils
{
	 ...
	 * @return array
	 */
	static function getOrderPropsVariant($orderPropsId)
	{
		if (($orderPropsId = intval($orderPropsId)) <= 0)  {
			return false;
		}
	 ...
	 * @return boolean
	 */
	static function setOrderProps($orderId, $arProps)
	{
		if (
	 ...
	 * @return array
	 */
	static function getCurrentBasketItems($arSelect = array(), $needBasketProps = false, $orderId = 'NULL')
	{
		\CModule::IncludeModule('sale');
		...
		if (intval($orderId) > 0 && $orderId != 'NULL') {
			$arF = array(
	 ...
	 * @return array
	 */
	static function getSaleOrderPropsDir()
	{
		static $_cache;
