 ...
 * ResizeWithHide() - Возвращает HTML с картинкой пропорщионально уменьшеной и вмещенной в дивку по центру с нужной шириной и высотой и overflow:hidden<br>
 * Combine() - склеивает несколько изображениий в одно<br>
...
class CImg{
	...
	public function methodFULL(){
		$this->raw = 3;
		if($this->w > $this->w_out){
			$this->w_fact = $this->w_out;
		} else{
			$this->w_fact = $this->w;
		}
		if($this->h > $this->h_out){
			$this->h_fact = $this->h_out;
		} else{
			$this->h_fact = $this->h;
		}
		if($this->w_out > $this->h_out){
			$this->w_fact = round($this->h_fact * $this->w / $this->h);
		} elseif($this->w_out < $this->h_out){
			$this->h_fact = round($this->w_fact * $this->h / $this->w);
		}
		$this->dy = ($this->h_out - $this->h_fact)/2;
		$this->dx = ($this->w_out - $this->w_fact)/2;
	}
...
//		imagealphablending($canv, true);
		foreach($imgs as $i){
			$mi = new self;
			$mi->load($i);
			$mi->imagecreatefrom();
			$mi->pathforbx();
//			imagealphablending($mi->res, true);
