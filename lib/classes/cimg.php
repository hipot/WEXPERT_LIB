<?
/**
 * -=Обработка изображений=-<br><br><b>Пример вызова</b><br>
 *
 * CImg::Resize($arItem['PREVIEW_PICTURE'], $width, $height)<br>
 * <i>все остальное за вас сделаю я</i><br><br><b>Methods</b><br>
 *
 ...
 * ResizeWithHide() - Возвращает HTML с картинкой пропорционально уменьшенной и
 * вмещенной в дивку по центру с нужной шириной и высотой и overflow:hidden<br>
 ...
 *
 * @author		&copy; WebExpert
 * @version		3.4, 23.03.2016
 */
class CImg
{
	/**
	...
	/**
	 * Имя тега, вызывать перед каждым вызовом трансформации
	 * @var string
	 */
	 ...
	 *
	 * @param $img Путь
	 *        	к картинке относительно корня сайта,<br> либо относительно корня документов, либо битрикс ID, либо ресурс изображения GD.<br>
	 *        	Если РЕСУРС, то необходимо указать $src путь к псевдо-картинке.
	 * @param $src Путь
	 *        	к псевдо-картинке. Абсолютный путь на диске либо относительно корня сайта.
	 * @return void
	 */
	...
	public function methodFULL()
	{
		...
		if ($this->h == $this->h_out && $this->w == $this->w_out) {
			...
			if ($this->w_fact > $this->w_out) {
				...
				$propScale = $prop_in_back;
				// fix version 3.01
				if ($this->h_fact > $this->w_fact) {
					$propScale = $prop_in;
				}

				$this->h_fact = round($this->w_fact * $propScale);
			}
	 ...
	 *
	 * @param bool $path
	 *        	Путь к файлу, не указывается, берется из префикса
	 ...
	 * <br>Можно задать свой новый метод <br>создав в классе новый метод <br>с названием - method[МЕТОД_ТРАНСФОРМАЦИИ] - methodMYMETHOD
	 */
	public function do_resize()
	{
		...
		if (!method_exists($this, $method)) {
			...
			if ($this->raw == 3 || $this->raw == 1) {
				...
				imagefill($this->des, 0, 0, imagecolorallocatealpha($this->des, 255, 255, 255, 0));
			}
			// ресемплируем
	...
	/**
	 * as Watermark.
	 * Накладывает изображение $img_src на загруженное изображение.
	 *
	 * @param $img_src -
	 *        	накладываемое изображение
	 * @param $pos(false-центр) -
	 *        	параметры наложения:<br>
	 *        	(str)lt: left top<br>
	...
	public function pathforbx($ssid = false)
	{
		...
		// @see http://blog.nexcess.net/2012/04/27/mkdir-too-many-links-what-it-is-and-how-to-fix/
		...
		$this->r_path = '/upload/cimg_cache/' . $cimgPath . '/' . $this->postfix . '/' . basename($this->src);
	...
	public function pathfor()
	{
		...
		/*if ($this->method == self::M_FULL || $this->method == self::M_FULL_S) {
			$this->r_path = str_replace(strrchr($this->r_path, "."), '.png', $this->r_path);
			$this->path = str_replace(strrchr($this->path, "."), '.png', $this->path);
		}*/
	}
	...
	public function imagesave($q = 80)
	{
		...
		if($this->raw == 1 && !in_array($this->method, array(self::M_FULL, self::M_FULL_S))){
			imagegif($this->des, $this->path);
		} elseif ($this->raw == 3 || $this->raw == 1) {
	...
	/**
	 * Почистит ресурсы изображений (освобождает память)
	 * @return void
	 */
	 ...
	 *
	 * @param string $path
	 *        	Путь к папке относительно корня документов
	 */
	 ...
	 *
	 * @param $f Картинка
	 *        	(rel_p|abs_p|bitrix_id)
	 * @param $w Ширина
	 * @param bool $h
	 *        	Высота (можно передать false для подгонки)
	 * @param string $m
	 * 			Метод трансформации ( CROP | CROP_TOP | FULL | FULL_S | STRETCH )<br>
	 *        	Можно задать свой новый метод <br>создав в классе новый метод <br>с названием - method[МЕТОД_ТРАНСФОРМАЦИИ] - methodMYMETHOD
	 * @param array $ch
	 *        	Проверять array(ширину, высоту) входящей картинки<br> чтоб не увеличивать маленькую.
	 * @param bool $sid =
	 *        	false Идентификатор сайта (указывается при многосайтовости)
	 * @param bool $ar =
	 *        	false Возврацать массив или строку. По умолчанию строку
	 * @return (array str) к картинке или массив (шир, выс, путь)
	 */
	public static function Resize($f, $w, $h, $m, $ch = array(false, false), $sid = false, $ar = false)
	{
		$mi = new self();
		$mi->load($f);
		$mi->w_out = $w;
	 ...
	 * @see CImg::Resize()
	 */
	public static function AResize($f, $w, $h, $m, $ch = array(false, false), $sid = false, $me = false)
	{
	 ...
	 * @param bool $h
	 *        	Высота
	 * @param string $alt = '' атрибут
	 *        	ALT картинки
	 * @param string $title = $alt атрибут
	 *        	TITLE картинки
	 * @param string array $ch = array(false,
	 *        	false) Проверять array(ширину, высоту) входящей картинки<br> чтоб не увеличивать маленькую.
	 * @param bool $sid =
	 *        	false Идентификатор сайта (указывается при многосайтовости)
	 ...
	 *
	 * @param $f Изображение(rel_p|abs_p|bitrix_id)
	 *        	на которое будет накладываться.
	 * @param $to Изображение
	 *        	которое будет накладываться(rel_p|abs_p|bitrix_id)
	 * @param (bool|array) $p=
	 *        	false - центр Позиция наложения<br>
	 *        	(str)lt: left top<br>
	 *        	(str)lb: left bottom<br>
	 *        	(str)rt: right top<br>
	 *        	(str)rb: right bottom<br>
	 ...
	 *        	false Идентификатор сайта (указывается при многосайтовости)
	 * @param (bool) $ar=
	 *        	false Возвращать ли массив или строку.
	 ...
	 *
	 * @param $f Картинка
	 *        	(rel_p|abs_p|bitrix_id) на которую будет накладываться
	 * @param $to Картинка
	 *        	(rel_p|abs_p|bitrix_id) которая будет накладываться
	 * @param bool $pos=
	 *        	false - центр. Позиция наложения<br>
	 *        	(str)lt: left top<br>
	 *        	(str)lb: left bottom<br>
	 ...
	 *        	Высота
	 * @param string $m
	 *        	Метод трансвформации.( CROP | CROP_TOP | FULL | STRETCH )<br>
	 *        	Можно задать свой новый метод <br>создав в классе новый метод <br>с названием - method[МЕТОД_ТРАНСФОРМАЦИИ] - methodMYMETHOD
	 * @param array $ch
	 *        	Проверять array(ширину, высоту) входящей картинки<br> чтоб не увеличивать маленькую.
	 * @param bool $sid
	 ...
	 * @return (array str) к картинке или массив (шир, выс, путь)
	 */
	public static function ResizeOverlay($f, $to, $pos, $w, $h, $m, $ch = array(true, true), $sid = false, $ar = false)
	{
		$mi = new self();
		$mi->load($f);
		$mi->w_out = $w;
	 ...
	 *
	 * @param array $imgs
	 *        	Массив склеиваемых изображний. Изображение под индексом 0 в самом низу(по его размерам и будет создана новая картинка) и так далее вверх
	 ...
	 *
	 * @param
	 *        	Скопируйте параметры метода класса CImg, за которым хотите прибраться.
	...
	public static function ClearCache()
	{
		...
		if (is_dir($params[0])) {
			...
			$path = '/upload/cimg_cache/' . $cimgPath . '/' . $postfix.'/';
		}
	 ...
	 * Для структурирования /upload/cimg_cache/<$tagName>/aaa/aaaaaaaaaaaaaaaaaaa/...
	 * При этом можно удобно удалять кеш через удаление папки /upload/cimg_cache/<$tagName>
